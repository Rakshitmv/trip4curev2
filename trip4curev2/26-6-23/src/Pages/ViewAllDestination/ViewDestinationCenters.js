...
const ViewDestinationCenters = () => {
    ...
    useEffect(() => {
        const fetchHospitals = async () => {
          try {
            const response = await fetch('http://13.234.216.30:8080/feature_destination_hosiptal/', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({ destination_country: country})
            });
                ...
                <Container>
                    <Row>
                        <div className='col-md-9 mx-auto mt-5 text-center'>
                            <h1 className='medical-center-heading'>Medical Centers in {country}</h1>
